...
- (NSDate*)end {
    ...
    // Guard: only compute duration-based fallback when start is actually set.
    // When start is nil (e.g. a freshly-allocated item in tests), messaging nil
    // in +dateWithTimeInterval:sinceDate: silently produces the Cocoa reference
    // date (2001-01-01), which misleads callers into thinking a date is set.
    if (!self.start) {
        return nil;
    }
