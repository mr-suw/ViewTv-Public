...
- (void)signalDidErrorDvrStore:(NSError*)error {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        // Post notification so that RecordingRepository (and any other
        // NSNotificationCenter-based observers) can react to fetch failures.
        [[NSNotificationCenter defaultCenter] postNotificationName:TVHDvrStoreDidErrorNotification
                                                            object:self];
    });
}
