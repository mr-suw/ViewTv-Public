...
- (void)cancelAllOperations {
    if ( _jsonClient ) {
        // NSURLSession pending tasks are cancelled automatically when jsonClient
        // is deallocated (TVHJsonClient.dealloc calls [session invalidateAndCancel]).
        // Nothing to do here beyond clearing the reference below.
    }
    [self stopTimer];
    if ( _cometStore ) {
