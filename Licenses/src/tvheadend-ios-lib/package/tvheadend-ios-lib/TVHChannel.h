...
// UIKit is available on tvOS and iOS, but not on macOS host builds (swift build CLI).
// Use conditional import + forward declarations so the library compiles on all hosts.
